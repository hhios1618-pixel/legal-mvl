...
import { ChevronLeft, MessageCircle, FileText, CalendarDays, Clock, CheckCircle, Download } from 'lucide-react';

const ClientCaseDetail: React.FC = () => {
  const { caseId } = useParams<{ caseId: string }>();
  const navigate = useNavigate();
  const { currentUser, cases, sendMessage, markMessagesAsRead } = useApp();
  
  const [newMessage, setNewMessage] = useState('');
  const [activeTab, setActiveTab] = useState<'timeline' | 'messages' | 'documents'>('timeline');

  const case_ = useMemo(() => {
    return cases.find(c => c.id === caseId);
  }, [cases, caseId]);

  if (!currentUser || !case_) {
    return (
                    ...
                    >
                      <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-xl shadow-sm ${
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-gray-400"><Clock className="w-3 h-3 inline-block mr-1" />{formatDate(message.timestamp)}</span>
                        </div>
