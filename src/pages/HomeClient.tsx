...
export default function HomeClient() {
  ...
  const create = useApp(s=>s.createCase);
    ...
    <main className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white font-semibold transition-all duration-200 shadow-lg hover:shadow-xl"
              >
                Acceso Abogado
              </button>
              <button
                onClick={() => navigate("/login?role=admin")}
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold transition-all duration-200 shadow-lg hover:shadow-xl"
              >
                Acceso Admin
              </button>
              <button
                onClick={() => navigate("/login?role=super_admin")}
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-violet-700 hover:from-violet-700 hover:to-violet-800 text-white font-semibold transition-all duration-200 shadow-lg hover:shadow-xl"
              >
                Super Admin
              </button>
            </motion.nav>
          ...
          {mobileMenuOpen && (
            ...
            >
              <nav className="flex flex-col gap-2">
                ...
                <button
                  onClick={() => navigate("/login?role=lawyer")}
                  className="mt-2 px-4 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-purple-700 text-white font-semibold transition-all text-center shadow-lg"
                >
                  Acceso Abogado
                </button>
                <button
                  onClick={() => navigate("/login?role=admin")}
                  className="mt-2 px-4 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-semibold transition-all text-center shadow-lg"
                >
                  Acceso Admin
                </button>
                <button
                  onClick={() => navigate("/login?role=super_admin")}
                  className="mt-2 px-4 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-violet-700 text-white font-semibold transition-all text-center shadow-lg"
                >
                  Super Admin
                </button>
              </nav>
      ...
      <IntakeModal
        ...
        onSubmit={(p)=>{
          create({
            type: p.type,
            title: `Consulta ${p.type}`,
            description: p.description || 'Consulta legal',
            priority: p.urgent ? 'urgente' : 'media',
